...
DROP TABLE IF EXISTS `MEMBER`;
CREATE TABLE MEMBER (
	-- ID(FK), 회원 아이디(FK), 비밀번호, 이름, 이메일, 주소, 통신사, 전화번호
                        ID INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY,
                        MEMBER_ID VARCHAR(15) NOT NULL,
                        PASSWORD VARCHAR(15),
                        NAME VARCHAR(15),
...
CREATE TABLE NOTICE (
	-- POST_ID(PK), COMPANY_ID(FK), 제목, 내용, 조회수, 게시날짜
                        ...
                        CONSTRAINT FK_NOTICE_COMPANY FOREIGN KEY (COMPANY_ID) REFERENCES COMPANY (COMPANY_ID)
);
DROP TABLE IF EXISTS `COMMENT`;
CREATE TABLE COMMENT (
...
CREATE TABLE ORDERS (
    ...
    -- 결제 방식 : 카드 CARD, 가상 계좌 VBANK, 무통장 입금 TRANS, 휴대폰 소액결제 PHONE, 카카오페이 KAKAO
                        ...
                        CONSTRAINT FK_ORDERS_MEMBER FOREIGN KEY (M_ID) REFERENCES MEMBER (ID) ON DELETE CASCADE,
						CONSTRAINT FK_ORDERS_COMPANY FOREIGN KEY (C_ID) REFERENCES COMPANY (ID) ON DELETE CASCADE,
						CONSTRAINT FK_ORDERS_GAME FOREIGN KEY (G_ID) REFERENCES GAME (ID) ON DELETE CASCADE,
                        CONSTRAINT DON_AMOUNT_CHECK CHECK (`DON_AMOUNT` >= 10000)
...
CREATE TABLE GAME (
                      ...
                      LIKE_CNT INT(10),
                      CONSTRAINT FK_GAME_COMPANY FOREIGN KEY (NAME) REFERENCES COMPANY (GAME) ON UPDATE CASCADE ON DELETE CASCADE
);